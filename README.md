...
- 虽然Nancy目前没有提供协议解析模块，但是我们仍希望能模拟长连接CGI的场景，来看看用Nancy搭建的网络服务的QPS。
- 同时，不加解析的模拟场景，也能看出抛开协议解析等外的纯网络IO性能。

实现原理

- 与pingpong测试类似，我们在服务端采用creactors，在客户端我们采用多个reactor（one loop per thread）。
- 与pingpong测试不同的是，客户端每次率先发送**4k的请求数据**给服务端，服务端**完全读取4k数据后**再返回16k的数据，并**确保这16k的数据**能被客户端**完全接收**。当这一系列的动作完成之后，客户端的request记数再加1，意为此次的request完成。（注意，我们这里禁用了**流水线**）
- 同时我们将每次交换的**20k**数据进行加总，和pingpong测试中数据的吞吐形成对照。
...
| 服务端线程数：2     |         |         |         |         |
| ------------------- | ------- | ------- | ------- | ------- |
| 并发数              | 500     | 1000    | 2000    | 5000    |
| QPS(q/s)            | 301450  | 245987  | 205088  | 197010  |
| Bytes(Mb/s)         | 5638.82 | 4804.5  | 4154.67 | 3848.09 |
|                     |         |         |         |         |
| **服务端线程数：4** |         |         |         |         |
| 并发数              | 500     | 1000    | 2000    | 5000    |
| QPS(q/s)            | 526242  | 400395  | 319599  | 255255  |
| Bytes(Mb/s)         | 10278.2 | 7820.42 | 6242.3  | 4985.68 |

结果说明：

- 在测试过程中我们没有可以保持客户端的线程数，以保持服务端的CPU占用拉满。如在服务端线程数为4时，客户端的线程数为8，否则会因为客户端的压力不够，导致结果不准确。但其余测试中，客户端的线程数维持在4条。

- 可以看到当服务端线程数为4时，在并发为500的时候达到最高的526242，数据的交换量达到10278Mb/s。
